...
def main():
    ...
    with st.form("kimi_k2_form"):
        ...
        submitted = st.form_submit_button("🚀 데이터 합성 시작", use_container_width=True)
    ...
    if submitted:
        if not synthesis_task.strip():
            ...
            command = [
                py_executable, "-m", "lang_graph.kimi_k2_agentic_data_synthesis.main",
                "--task", synthesis_task,
                "--data-type", data_type,
            ...
            result = run_agent_process(
                ...
                command=command,
                process_key_prefix="logs/kimi_k2"
            )
