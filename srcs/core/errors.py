...
def handle_data_processing_error(data_item: Any, operation: Callable, default_result: Any = None) -> Any:
    """
    Safely apply an operation to data_item with standardized error handling.

    Args:
        data_item: The data item being processed
        operation: Callable to apply to data_item
        default_result: Default result to return on error

    Returns:
        operation(data_item) or default_result if error occurs
    """
    try:
        ...
        try:
            ...
            logger.warning("Data processing error", operation=operation.__name__, error=str(e))
        except ImportError:
            pass
