...
class BackgroundMemoryService:
    ...
    def __init__(
        ...
        logger.info(f"BackgroundMemoryService initialized: max_concurrent={max_concurrent_tasks}")
...
def get_background_memory_service() -> BackgroundMemoryService:
    """전역 백그라운드 메모리 서비스 인스턴스 반환."""
    global _background_memory_service
    if _background_memory_service is None:
        _background_memory_service = BackgroundMemoryService()
