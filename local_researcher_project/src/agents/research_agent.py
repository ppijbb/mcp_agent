...
class ResearchAgent:
    ...
    async def _llm_conduct_research(self, task: Dict[str, Any], objective_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Use task description from the task object
            task_description = task.get('description', 'Unknown task')
            return await self._llm_autonomous_research(task_description)
    ...
    async def _get_llm_research_plan(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Enable LLM to use MCP tools for actual research
            return await self._llm_autonomous_research(task_description)
    
    async def _llm_autonomous_research(self, task_description: str) -> Dict[str, Any]:
        """LLM performs autonomous research using available MCP tools."""
        try:
            if not self.llm:
                # Generate basic research plan without LLM
                return await self._generate_basic_research_plan(task_description)
            
            # LLM decides what tools to use and how to conduct research
            research_prompt = f"""
You are an autonomous research agent. Your task is to conduct comprehensive research on: "{task_description}"
...
Your mission:
1. ACTIVELY use available tools to gather real information
2. Perform critical analysis of findings
3. Validate information through multiple sources
4. Synthesize insights and draw conclusions
5. Provide evidence-based recommendations

Generate a detailed research plan with specific tool calls and analysis steps.
Format your response as JSON with this structure:
{{
    "research_strategy": "description of your approach",
    "tool_calls": [
        {{
            "tool": "web_search",
    ...
    "analysis_plan": [
        {{
            ...
            "method": "how you will analyze the data"
        }}
    ],
...
"""
            ...
            response = await self.llm.generate_content_async(research_prompt)
            
            # Parse LLM response and execute the research plan
            import json
            import re
            ...
            if json_match:
                ...
                # Execute the LLM-generated research plan
                research_results = await self._execute_llm_research_plan(research_plan, task_description)
                return research_results
            else:
                return {'steps': [], 'method': 'llm_autonomous', 'error': 'Failed to parse LLM research plan'}
                
        except Exception as e:
            logger.error(f"LLM autonomous research failed: {e}")
            return {'steps': [], 'method': 'llm_autonomous', 'error': str(e)}
    
    async def _execute_llm_research_plan(self, research_plan: Dict[str, Any], task_description: str) -> Dict[str, Any]:
        """Execute the research plan generated by LLM."""
        try:
            results = {
                'strategy': research_plan.get('research_strategy', ''),
                'findings': [],
                'analysis': [],
                'validation': [],
                'conclusions': [],
                'method': 'llm_autonomous'
            }
            
            # Execute tool calls as planned by LLM
            for tool_call in research_plan.get('tool_calls', []):
                tool_result = await self._execute_llm_tool_call(tool_call)
                if tool_result:
                    results['findings'].append({
                        'source': tool_call.get('tool', 'unknown'),
            ...
            for analysis_step in research_plan.get('analysis_plan', []):
                analysis_result = await self._perform_llm_analysis(analysis_step, results['findings'])
                if analysis_result:
                    results['analysis'].append(analysis_result)
            
            # LLM validates findings
            validation_results = await self._llm_validate_findings(results['findings'], research_plan.get('validation_criteria', []))
            results['validation'] = validation_results
            
            # LLM generates conclusions
            conclusions = await self._llm_generate_conclusions(results, task_description)
            results['conclusions'] = conclusions
            
            return results
            
        except Exception as e:
            logger.error(f"LLM research plan execution failed: {e}")
            return {'method': 'llm_autonomous', 'error': str(e)}
    
    async def _execute_llm_tool_call(self, tool_call: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Execute a specific tool call as directed by LLM."""
        try:
            tool = tool_call.get('tool', '')
            query = tool_call.get('query', '')
            
            if tool == 'web_search':
                logger.info(f"LLM requested web search: {query}")
                return await self._perform_web_search(query)
            elif tool == 'academic_search':
                logger.info(f"LLM requested academic search: {query}")
                return await self._perform_academic_search(query)
            elif tool == 'data_analysis':
                logger.info(f"LLM requested data analysis: {query}")
                return await self._perform_data_analysis(query, tool_call)
            else:
                logger.warning(f"Unknown tool requested by LLM: {tool}")
                # Default to web search for unknown tools
                return await self._perform_web_search(query)
                
        except Exception as e:
            logger.error(f"LLM tool call execution failed: {e}")
            return None
    
    async def _perform_llm_analysis(self, analysis_step: Dict[str, Any], findings: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
        """Perform analysis as directed by LLM."""
        try:
            if not self.llm:
                return None
                
            analysis_prompt = f"""
Perform the following analysis step: {analysis_step.get('step', '')}
Method: {analysis_step.get('method', '')}
...
{{
    ...
    "critical_evaluation": "your critical assessment",
    "limitations": ["limitation1", "limitation2"],
    "recommendations": ["recommendation1", "recommendation2"]
}}
"""
            
            response = await self.llm.generate_content_async(analysis_prompt)
            ...
            if json_match:
                ...
                return {'analysis_type': 'text', 'content': response.text}
                
        except Exception as e:
            logger.error(f"LLM analysis failed: {e}")
            return None
    
    async def _llm_validate_findings(self, findings: List[Dict[str, Any]], criteria: List[str]) -> List[Dict[str, Any]]:
        """LLM validates research findings against criteria."""
        try:
            if not self.llm or not findings:
                return []
                
            validation_prompt = f"""
Validate the following research findings against these criteria:
{json.dumps(criteria)}
...
6. Overall validation score (0-1)
...
[
    {{
        ...
        "source_reliability": "evaluation", 
        "completeness": "check result",
        "bias_detection": "bias analysis",
        "cross_reference": "verification result",
...
"""
            ...
            response = await self.llm.generate_content_async(validation_prompt)
    ...
    async def _llm_generate_conclusions(self, research_results: Dict[str, Any], task_description: str) -> List[Dict[str, Any]]:
        """LLM generates evidence-based conclusions."""
        try:
            if not self.llm:
                return []
                
            conclusion_prompt = f"""
Based on the comprehensive research conducted for: "{task_description}"

Research Results:
{json.dumps(research_results, indent=2)}

Generate evidence-based conclusions that include:
...
[
    {{
        "conclusion_type": "main_finding/insight/recommendation",
        "statement": "clear conclusion statement",
...
"""
            ...
            response = await self.llm.generate_content_async(conclusion_prompt)
            ...
            if json_match:
                ...
                return [{'conclusion_type': 'text', 'statement': response.text}]
    ...
    async def _execute_research_step(self, step: Dict[str, Any]) -> Dict[str, Any]:
        """Execute a single research step using browser manager.
        ...
        try:
            ...
            if action == "navigate":
                url = step.get("url")
                if url:
                    # Use browser manager for navigation and extraction
                    result = await self.browser_navigate_and_extract(url, goal)
                    return {
                        "action": action,
                        "url": url,
                        "goal": goal,
                        "data_collected": [result] if result.get("success") else [],
                ...
                if query:
                    # Use browser manager for search and extraction
                    results = await self.browser_search_and_extract(query, goal, 5)
                    return {
                        "action": action,
                        "query": query,
                        "goal": goal,
                ...
                # Extract content using browser manager
                url = step.get("url", "")
                if url:
                    result = await self.browser_navigate_and_extract(url, goal)
                    return {
