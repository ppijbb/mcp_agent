...
class MarkdownGenerator:
    ...
    def generate(self, data: Dict[str, Any]) -> Document:
        ...
        try:
            ...
            return self._create_fallback_document(data)
    ...
    def _create_fallback_document(self, data: Dict[str, Any]) -> Document:
        """Create a fallback document."""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"research_report_fallback_{timestamp}.md"
        file_path = Path(self.output_directory) / filename
        
        content = f"""# Research Report: {data.get('topic', 'Unknown Topic')}
...
---
*Generated by Local Researcher (Fallback)*
"""
