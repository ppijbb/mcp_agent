...
async def _call_tools_concurrently_async(tool_name: str, tickers: List[str], extra_kwargs: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    try:
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                tasks = [
                    _call_tool_async(session, tool_name, {"ticker": ticker, **kwargs})
...
def _run_async_concurrent_calls(tool_name: str, tickers: List[str], extra_kwargs: Optional[Dict] = None) -> Dict[str, Any]:
    """동기 함수에서 비동기 병렬 MCP 호출을 실행하기 위한 래퍼"""
    return asyncio.run(_call_tools_concurrently_async(tool_name, tickers, extra_kwargs))
...
def call_ohlcv_data_tool(tickers: List[str], period: str = None) -> Dict:
    ...
    extra_kwargs = {"period": period} if period else None
    return _run_async_concurrent_calls("get_ohlcv_data", tickers, extra_kwargs) 