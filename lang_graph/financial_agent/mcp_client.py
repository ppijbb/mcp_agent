...
async def _call_tool_async(session: ClientSession, tool_name: str, arguments: Dict) -> Any:
    ...
    # 텍스트 콘텐츠 블록을 모두 추출합니다 (빈 콘텐츠/텍스트 외 콘텐츠에도 안전).
    content = result.content or []
    texts = [block.text for block in content if hasattr(block, "text")]
    if texts:
        text = "\n".join(texts)
    else:
        # 텍스트 콘텐츠가 없으면 structuredContent로 폴백하고, 그마저 없으면 오류를 발생시킵니다.
...
async def _call_tools_concurrently_async(tool_name: str, tickers: List[str], extra_args: Dict = None) -> Dict[str, Any]:
    ...
    try:
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                tasks = [
                    _call_tool_async(session, tool_name, {"ticker": ticker, **extra})
...
def _run_async_concurrent_calls(tool_name: str, tickers: List[str], extra_args: Dict = None) -> Dict[str, Any]:
    """동기 함수에서 비동기 병렬 MCP 호출을 실행하기 위한 래퍼"""
    return asyncio.run(_call_tools_concurrently_async(tool_name, tickers, extra_args))
...
def call_ohlcv_data_tool(tickers: List[str], period: str = None) -> Dict:
    ...
    extra_args = {"period": period} if period else None
    return _run_async_concurrent_calls("get_ohlcv_data", tickers, extra_args)