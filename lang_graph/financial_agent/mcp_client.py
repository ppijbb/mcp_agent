...
async def _call_tools_concurrently_async(tool_name: str, tickers: List[str], extra_args: Dict[str, str] = None) -> Dict[str, Any]:
    ...
    try:
        async with stdio_client(server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                tasks = [
                    _call_tool_async(session, tool_name, {"ticker": ticker, **(extra_args or {})})
...
def call_ohlcv_data_tool(tickers: List[str], period: str = None) -> Dict:
    ...
    return _run_async_concurrent_calls_with_args("get_ohlcv_data", tickers, extra_args)


def _run_async_concurrent_calls_with_args(tool_name: str, tickers: List[str], extra_args: Dict[str, str] = None) -> Dict[str, Any]:
    """동기 함수에서 비동기 병렬 MCP 호출을 실행하기 위한 래퍼 (추가 인자 지원)"""
