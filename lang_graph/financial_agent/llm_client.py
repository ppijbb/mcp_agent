"""
LLM Client Module for Financial Agent

Provides LLM client initialization and invocation utilities for the financial agent.
Uses LangChain for model abstraction and supports multiple model providers.

Variables:
    llm: Global BaseChatModel instance
    model_manager: Global ModelManager instance

Functions:
    initialize_llm_client: Initialize the LLM client with configuration
    call_llm: Invoke the LLM with a prompt
"""
...
def initialize_llm_client() -> None:
    ...
    Loads multi-model configuration, creates a ModelManager instance,
    and initializes the LLM client based on preferred provider settings.

    Raises:
        RuntimeError: If no available LLM model is found or initialization fails.
...
def call_llm(prompt: str) -> str:
    """
    Invoke the LLM with a given prompt and return the response as a string.
    ...
    Raises:
        RuntimeError: If the LLM client is not initialized or invocation fails.
    """
    if llm is None:
